...
import com.group2.kelem.dao.VoteRepository;
...
import com.group2.kelem.services.AnswerService;
...
public class AnswerController {
   ...
   private UserModel loggedInUser(){
    ...
    public String answered(@PathVariable String id, @Valid @ModelAttribute("answer") AnswerModel answer, Errors errors, Model model) {
        ...
        return "redirect:/just-answered/" + q.getId();
    }
    ...
    public String allAnswer(Model model, @PathVariable Long id) {
        ...
        for (AnswerModel am: answer) {
            upVote.put(am, voteRepository.findUpVote(answer.get(0).getId()));
            downVote.put(am, voteRepository.findDownVote(answer.get(0).getId()));
        }
